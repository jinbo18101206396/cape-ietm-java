...
public class IetmStandardController extends JeecgController<IetmStandard, IIetmStandardService> {
     ...
     * @return
     */
    @ApiOperation(value="获取标准树形结构", notes="获取标准树形结构")
    @GetMapping(value = "/treeList")
    public Result<List<IetmStandard>> queryTreeList() {
        List<IetmStandard> list = ietmStandardService.list();
     ...
     * @return
     */
    @ApiOperation(value="根据ID查询标准详情", notes="根据ID查询标准详情")
    @GetMapping(value = "/queryById")
    public Result<IetmStandard> queryById(@RequestParam(name="id",required=true) String id) {
        IetmStandard ietmStandard = ietmStandardService.getById(id);
        if(ietmStandard == null) {
            return Result.error("未找到对应数据");
        }
        return Result.OK(ietmStandard);
    }
