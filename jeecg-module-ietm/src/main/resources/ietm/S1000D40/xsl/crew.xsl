...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!--===============================================-->
	<xsl:template match="descacrw | descrCrew | drill | crewDrill | procd/para | step | frc | crewRefCard | subdrill | subCrewDrill | challrsp | challengeAndResponse">
		<xsl:apply-templates/>
	</xsl:template>
	<!--====================2012定点样式范例===========================-->
  ...
  <!--=====================2012加上crewDrillStep=========== name="T_StepParaContent"============font-size:"10.5pt";vertical-align:"left"; line-height:"2.2";===-->
	<xsl:template match="challeng | challenge | procd | crewProcedureName/para | step/title | crewDrillStep/title">
     ...
     <xsl:variable name="indent">
            <xsl:choose>
					  ...
					  <xsl:value-of select="(count(ancestor::elseIf))  + (count(ancestor::elseif))  + (count(ancestor::step)) + (count(ancestor::crewDrillStep)) + (count(ancestor::crewDrill))+ (count(ancestor::subCrewDrill))+ (count(ancestor::challengeAndResponse))+ (count(ancestor::if)-1) + (count(ancestor::case)) + (count(ancestor::caseCond))-1"/>
					  </xsl:when>
					  <xsl:when test="(self::procd or self::crewProcedureName or self::title or self::para) and count(ancestor::crewDrillStep)>1">
					  <xsl:value-of select="(count(ancestor::if  | ancestor::case |  ancestor::caseCond | ancestor::elseif | ancestor::elseIf | ancestor::step[not(position()=1)] | ancestor::crewDrillStep[not(position()=1)]))-1"/></xsl:when>
					  <xsl:otherwise>2</xsl:otherwise>
			      </xsl:choose>
        </xsl:variable>
			<div>
			<xsl:attribute name="style">margin-left: <xsl:value-of select="$indent"/>em; width:90%;</xsl:attribute>
                ...
                <xsl:when test="((self::title or not(preceding-sibling::title)) and
								not(preceding-sibling::para or preceding-sibling::challeng or preceding-sibling::challenge or preceding-sibling::procd)) or
                          ((self::challengeAndResponse and not(preceding-sibling::para)) and
            ...
            <!--<xsl:apply-templates/>-->
        &#160;<xsl:value-of select="."/>&#160;——&#160;<xsl:value-of select="../response"/>
			</div>
		</div>
	</xsl:template>
	<!-- ===================================================================== -->
