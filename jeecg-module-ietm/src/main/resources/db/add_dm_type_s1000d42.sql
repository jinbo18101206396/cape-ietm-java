-- =============================================================================
-- 为 ietm_dm_type 补齐 S1000D4.2 整套类型（数据库：DM8 / IETM schema）
-- 背景：类型表缺 S1000D4.2。若有项目配 S1000D4.2，其下建 DM 时 resolveDmType 的
--       ietm_standard 等值条件匹配不中，无论什么类型都回退默认 descript.xsd。
-- 前置确认：S1000D4.2 的 7 类 XSD + 模板文件在 classpath ietm/S1000D42/ 下全部存在。
...
-- ---- 1 插入前核对 ----
-- 1.1 确认 S1000D4.2 目前无数据（应返回 0）
SELECT COUNT(*) AS s42_cnt FROM ietm_dm_type WHERE ietm_standard = 'S1000D4.2';
-- 1.2 确认 S1000D4.0 源数据存在（应返回 8）
SELECT COUNT(*) AS s40_cnt FROM ietm_dm_type WHERE ietm_standard = 'S1000D4.0';

-- ---- 2 插入：复制 S1000D4.0 → S1000D4.2 ----
...
SELECT
  'S1000D42' || LPAD(TO_CHAR(ROWNUM), 3, '0'),
  type_code,
  type_name,
  type_name_en,
  'S1000D4.2',
...
-- ---- 3 复核：S1000D4.2 应有 8 行，type_code 与 4.0 一致（maintPlanning 已保持）----
SELECT type_code, type_name, ietm_standard, xsd_file, template_file, status
FROM ietm_dm_type
WHERE ietm_standard = 'S1000D4.2'
ORDER BY sort_no;
...
-- =============================================================================
-- 回滚（仅 COMMIT 之前有效）
-- DELETE FROM ietm_dm_type WHERE ietm_standard = 'S1000D4.2';
-- COMMIT;
-- =============================================================================
